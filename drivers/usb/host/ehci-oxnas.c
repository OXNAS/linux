...
static int ehci_oxnas_drv_probe(struct platform_device *ofdev)
{
	...
	oxnas->regmap =
		syscon_regmap_lookup_by_phandle(np, "oxsemi,sys-ctrl");
	...
	oxnas->input_pol_invert =
		of_property_read_bool(np, "oxsemi,input-pol-invert");
	oxnas->output_pol_invert =
		of_property_read_bool(np, "oxsemi,output-pol-invert");
	oxnas->patch_detect =
		of_property_read_bool(np, "oxsemi,patech-detect");
